layui.use(['table','layer',"form"],function(){
    ...
    table.on('toolbar(customers)',function (obj) {
        switch (obj.event) {
            ...
            case "order":
                ...
                break;
            case "link":
                openLinkManDialog(table.checkStatus(obj.config.id).data);
                break;
            case "recode":
                openRecodeDialog(table.checkStatus(obj.config.id).data);
                break;
        }
    });
    ...
    function openLinkManDialog(data){
       var title = "客户管理-联系人信息查看与添加修改"
        if(data.length==0){
            layer.msg("请选择查看联系人信息对应客户!");
            return;
        }
        if(data.length>1){
            layer.msg("暂不支持批量查看!");
        ...
        layui.layer.open({
            ...
            area:["700px","500px"],
            maxmin:true,
            content:ctx+"/customer/linkManInfo?cid="+data[0].id
        })
    }

    function openRecodeDialog(data){
        var title = "客户管理-交往记录信息查看"
        if(data.length==0){
            layer.msg("请选择查看联系人信息对应客户!");
            return;
        }
        ...
        layui.layer.open({
            ...
            area:["700px","500px"],
            maxmin:true,
            content:ctx+"/customer_concat/to_customer_contact?cid="+data[0].id
        })
    }
